...
class DozorClassicPrequel(GameEngine):
    name = 'DozorClassicPrequel'
    ...
    @staticmethod
    def start(config):
        session = DozorClassicPrequel.obtain_session(config)
        if session is None:
            return
    ...
    @staticmethod
    def get():
        return GameEngine.get(DozorClassicPrequel)

    def __init__(self, game):
        super().__init__(game)
        self.link = 'https://classic.dzzzr.ru/%s/?section=anons&league=%s' % (self.game.city, self.game.league)
        self._headers={
